...
public class DeleteClinicTest {
    ...
    public void testIfClinicDeletedSuccessfullyForUserWithAdminRole() throws Exception {
        long count = clinicRepository.count();

        mockMvc.perform(delete("/api/v1/admin/clinic/1")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());

        mockMvc.perform(get("/api/v1/admin/clinics"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.clinics[0].name").value("Clinic 2"))
                .andExpect(jsonPath("$.clinics.length()").value(count - 1));
    }
}
